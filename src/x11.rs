    ...
    fn handle_event(
        ...
        match event {
            xcb::Event::X(xcb::x::Event::MapRequest(e)) => {
                debug!("map request for window {}", e.window);
                canvas.add_window(e.window, cfg)?;
            }
            xcb::Event::X(xcb::x::Event::UnmapNotify(e)) => {
                debug!("unmap notify for window {}", e.window);
                canvas.remove_window(e.window)?;
            }
            xcb::Event::X(xcb::x::Event::DestroyNotify(e)) => {
                debug!("destroy notify for window {}", e.window);
                canvas.remove_window(e.window)?;
            }
